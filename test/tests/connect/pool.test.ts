...
import { PgPool } from "@asla/pg";

const test = connectionTest.extend<{ pool: PgPool }>({
  async pool({ connect }, use) {
    const pool = new PgPool({ create: async () => connect, maxCount: 1 });
    try {
      await use(pool);
    } finally {
...
test("游标批读完成、提前退出和显式关闭归还", async ({ pool }) => {
  const cursor = pool.open("SELECT generate_series(1, 3) AS value", { iteratorMaxRows: 1 });
  expect(await cursor.read(1)).toEqual([{ value: 1 }]);
  expect(pool.idleCount).toBe(0);
  await cursor.close();
  await cursor.close();
  expect(pool.idleCount).toBe(1);
  const complete = pool.open("SELECT 1 AS value");
  expect(await Array.fromAsync(complete)).toEqual([{ value: 1 }]);
  expect((await complete.completion).status).toBe("complete");
  const early = pool.open("SELECT generate_series(1, 4)", { iteratorMaxRows: 1 });
...
test("事务结束自动关闭未读完的游标", async ({ pool }) => {
  const tx = pool.begin();
  const cursor = tx.open("SELECT generate_series(1, 3)");
  await cursor.read(1);
  await tx.rollback();
