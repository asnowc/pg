...
import { PgTransactionStatus } from "@/protocol.ts";
...
test("事务提交持久化，普通连接查询在事务结束后执行", async ({ connect }) => {
  ...
  await tx.commit();
  expect(() => tx.open("SELECT 1")).toThrow("released");
});
...
test("事务回滚关闭未耗尽游标并释放连接队列", async ({ connect }) => {
  await using tx = connect.begin();
  const cursor = tx.open<{ n: number }>("SELECT generate_series(1, 5)::int AS n");
...
test("事务游标完整读取并提交", async ({ connect }) => {
  await using tx = connect.begin();
  await using cursor = tx.open<{ n: number }>("SELECT generate_series(1, 3)::int AS n", { iteratorMaxRows: 2 });
