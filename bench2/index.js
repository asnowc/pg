...
const series = (fn, xs) => {
  const results = [];
  return xs.reduce(
    (acc, x) => acc.then(() => fn(x).then((r) => results.push(r))),
    Promise.resolve({}),
  ).then(() => results);
};
...
series(test, clients).then((results) => {
  ...
  queries.forEach((q, i) =>
    results.map((x) => x.queries[i]).sort((a, b) => b.time - a.time).forEach(
      (x, i, xs) => {
  ...
  console.log(
    [pad(1, 10, " ")("client")].concat(queries.map(pad(0, 14, " "))).join(
      " | ",
    ),
  );
  console.log(
    [pad(1, 10, "-")(":")].concat(queries.map((x) => ":").map(pad(0, 14, "-")))
      .join(" | "),
  );
  results.forEach((x) =>
    console.log(
  ...
  results.forEach((x) =>
    console.log(
      [x.client].concat(x.queries.map((x) => x.time.toFixed(3))).join(";")
        .replace(/\./g, ","),
    )
  );
...
async function test(clientName, done) {
  return new Promise(async (resolve) => {
    const client = await import("./" + clientName + "/index.js");
    ...
    function next(x) {
      ...
      q.length
        ...
        : Promise.resolve(client.end()).then(() =>
          resolve({ client: clientName, queries: client.queries })
        );
    }
